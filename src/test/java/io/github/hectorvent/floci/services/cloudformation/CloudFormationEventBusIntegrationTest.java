...
class CloudFormationEventBusIntegrationTest {
    ...
    @Test
    void legacyTrackedBusWithoutOwnershipMarkerMigratesOnUpdate() throws InterruptedException {
        String suffix = Long.toString(System.nanoTime(), 36);
        String busName = "legacy-update-bus-" + suffix;
        String stackName = "eventbus-legacy-update-" + suffix;

        createStack(stackName, eventBusOnlyTemplate(busName, "before"));
        assertStackStatus(stackName, "CREATE_COMPLETE");

        StackResource busResource = cloudFormationService
                .describeStackResources(stackName, "us-east-1").stream()
                .filter(resource -> "Bus".equals(resource.getLogicalId()))
                .findFirst()
                .orElseThrow();
        busResource.getAttributes().remove("FlociEventBusCreatedTime");
        busResource.getAttributes().remove("FlociEventBusNameMode");

        updateStack(stackName, eventBusOnlyTemplate(busName, "after"));
        assertStackStatus(stackName, "UPDATE_COMPLETE");
        callEventBridge("AWSEvents.DescribeEventBus",
                "{\"Name\":\"" + busName + "\"}", 200);
