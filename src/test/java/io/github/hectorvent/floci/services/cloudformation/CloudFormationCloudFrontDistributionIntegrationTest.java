...
class CloudFormationCloudFrontDistributionIntegrationTest {
    ...
    void createStackProvisionsBrowsableDistributionWithResolvedDomainName() {
        ...
        String alias = "cfn-alias-" + suffix + ".example.test";
    ...
    @Test
    void updateStackMigratesLegacyPlaceholderDistributionId() {
        String suffix = Long.toString(System.nanoTime(), 36);
        String bucket = "cfn-cf-legacy-" + suffix;
        String alias = "cfn-legacy-" + suffix + ".example.test";
        String stackName = "cfn-cloudfront-legacy-" + suffix;
        s3Service.createBucket(bucket, "us-east-1");
        ...
        Distribution original = distributionsWithAlias(alias).getFirst();
        cloudFrontService.removeDistribution(original.getId());
        cloudFormationService.describeStackResources(stackName, "us-east-1").stream()
                .filter(resource -> "Dist".equals(resource.getLogicalId()))
                .findFirst()
                .orElseThrow()
                .setPhysicalId("Dist-deadbeef");
        ...
        assertEquals("after-" + suffix, matching.getFirst().getConfig().getComment());
        assertEquals(matching.getFirst().getId(),
                cloudFormationService.describeStackResources(stackName, "us-east-1").stream()
                        .filter(resource -> "Dist".equals(resource.getLogicalId()))
                        .findFirst()
                        .orElseThrow()
