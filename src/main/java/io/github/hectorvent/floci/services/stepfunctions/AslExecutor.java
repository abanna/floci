...
public class AslExecutor {
    ...
    // ecs:runTask.sync polling. AWS waits for the ECS task to reach a terminal state; an
    // implementation-specific wall-clock cap would turn valid long-running tasks into false
    // States.Timeout failures.
    ...
    private void doExecute(StateMachine sm, Execution exec, List<HistoryEvent> history,
        ...
        try {
            ...
            while (currentStateName != null) {
                ensureExecutionRunning(exec.getExecutionArn());
            ...
            LOG.debugv("ASL execution stopped for {0}", exec.getExecutionArn());
        } catch (Exception e) {
            LOG.warnv("ASL execution failed for {0}: {1}", exec.getExecutionArn(), e.getMessage());
            if (!isExecutionRunning(exec.getExecutionArn())) {
                return;
            }
            exec.setStatus("FAILED");
            exec.setStopDate(System.currentTimeMillis() / 1000.0);
    ...
    private boolean isExecutionRunning(String executionArn) {
        if (executionArn == null || executionArn.isBlank()
                || sfnService == null || !sfnService.isResolvable()) {
            return true;
        }
        return sfnService.get().isExecutionRunning(executionArn);
    }

    private void ensureExecutionRunning(String executionArn) {
        if (!isExecutionRunning(executionArn)) {
    ...
    private StateResult executeTaskState(String stateName, JsonNode stateDef, JsonNode input,
        ...
        if (needsToken) {
            ...
            tokenFuture = sfnService.get().registerPendingToken(taskToken, executionArn);
        }
        ...
        if (jsonata) {
            ...
            taskResult = invokeResource(
                    effectiveResource, effectiveInput, sm, taskToken, executionArn);
        } else {
            JsonNode effectiveInput = applyInputPath(stateDef, input);
            if (stateDef.has("Parameters")) {
        ...
        if (tokenFuture != null) {
            taskResult = awaitToken(taskToken, tokenFuture, stateDef);
        }
    ...
    private JsonNode awaitToken(String taskToken, CompletableFuture<JsonNode> future,
                                JsonNode stateDef) throws Exception {
        ...
        long taskDeadline = startedAt + TimeUnit.SECONDS.toNanos(timeoutSeconds);

        while (true) {
            long heartbeatDeadline = Long.MAX_VALUE;
            if (heartbeatSeconds > 0) {
                long lastHeartbeat = sfnService.get()
                        .lastTaskHeartbeatNanos(taskToken, future);
                if (lastHeartbeat >= 0) {
                    heartbeatDeadline = lastHeartbeat
                            + TimeUnit.SECONDS.toNanos(heartbeatSeconds);
                }
            }
            long deadline = Math.min(taskDeadline, heartbeatDeadline);
            long remaining = deadline - System.nanoTime();
            if (remaining <= 0) {
                if (future.isDone()) {
                    return future.get();
                }
                sfnService.get().timeoutPendingToken(taskToken, future);
                boolean heartbeatExpired = heartbeatDeadline <= taskDeadline;
                String error = heartbeatExpired ? "States.HeartbeatTimeout" : "States.Timeout";
                String timeoutKind = heartbeatExpired ? "heartbeat" : "task";
                throw new FailStateException(error,
                        "Task " + timeoutKind + " timed out");
            }
            try {
                return future.get(remaining, TimeUnit.NANOSECONDS);
            } catch (java.util.concurrent.TimeoutException e) {
                // A heartbeat may have moved the heartbeat deadline while this wait was in progress.
    ...
    private JsonNode invokeResource(String resource, JsonNode input, StateMachine sm, String taskToken,
                                    String executionArn) throws Exception {
        ...
        // AWS SDK service integrations: Step Functions task-token callbacks.
        if (resource.equals("arn:aws:states:::aws-sdk:sfn:sendTaskSuccess")
                || resource.equals("arn:aws:states:::aws-sdk:sfn:sendTaskFailure")
                || resource.equals("arn:aws:states:::aws-sdk:sfn:sendTaskHeartbeat")) {
            String callbackToken = input.path("TaskToken").asText(null);
            if (callbackToken == null || callbackToken.isBlank()) {
                throw new FailStateException("Sfn.InvalidToken", "TaskToken is required");
            }
            try {
                if (resource.endsWith(":sendTaskSuccess")) {
                    JsonNode outputNode = input.get("Output");
                    String callbackOutput = outputNode == null || outputNode.isNull()
                            ? null
                            : outputNode.isTextual() ? outputNode.asText() : outputNode.toString();
                    sfnService.get().sendTaskSuccess(callbackToken, callbackOutput);
                } else if (resource.endsWith(":sendTaskFailure")) {
                    sfnService.get().sendTaskFailure(
                            callbackToken,
                            input.path("Cause").asText(null),
                            input.path("Error").asText("States.TaskFailed"));
                } else {
                    sfnService.get().sendTaskHeartbeat(callbackToken);
                }
            } catch (AwsException e) {
        ...
        if (resource.startsWith("arn:aws:states:::ecs:runTask")) {
            ...
            String region = extractRegionFromArn(sm.getStateMachineArn());
            return invokeEcsRunTask(mode, input, region, executionArn);
        }
        ...
        if (resource.startsWith("arn:aws:states:::states:startExecution")) {
            ...
            String region = extractRegionFromArn(sm.getStateMachineArn());
            return invokeNestedStateMachine(mode, input, region, executionArn);
        }
    ...
    private JsonNode invokeNestedStateMachine(String mode, JsonNode input, String region,
                                              String parentExecutionArn) throws Exception {
        ...
        // .sync or .sync:2 — poll until terminal. AWS does not impose an implicit
        // one-minute limit on nested executions; the Task/state-machine timeout,
        // when configured, owns that lifecycle.
        while (true) {
            ensureExecutionRunning(parentExecutionArn);
            Thread.sleep(100);
            ensureExecutionRunning(parentExecutionArn);
    ...
    private JsonNode invokeEcsRunTask(String mode, JsonNode input, String region,
        ...
        while (true) {
            ensureExecutionRunning(executionArn);
        ...
        try {
            for (Future<JsonNode> future : futures) {
                ...
                try {
                    ...
                    Throwable cause = e.getCause();
                    if (cause instanceof FailStateException failure) {
                        throw failure;
                    }
    ...
    private StateResult executeMapState(String name, JsonNode stateDef, JsonNode input, StateMachine sm,
        ...
        JsonNode[] childInputValues = hasResultWriter ? new JsonNode[itemCount] : null;
        long[][] childTimingValues = hasResultWriter ? new long[itemCount][] : null;
        ...
        java.util.function.IntFunction<Callable<JsonNode>> makeTask = (i) -> () -> {
            ...
            JsonNode branchOutput = executeBranch(startAt, iteratorStates, iterInput, sm, topLevelQueryLanguage,
                    iterContext, variables.deepCopy());
            if (hasResultWriter) {
                childInputValues[i] = iterInput.deepCopy();
                childTimingValues[i] = new long[]{startMs, System.currentTimeMillis()};
            }
            return branchOutput;
        };
        ...
        ArrayNode childInputs = null;
        List<long[]> childTimings = null;
        if (hasResultWriter) {
            childInputs = objectMapper.createArrayNode();
            childTimings = new ArrayList<>(itemCount);
            for (int i = 0; i < itemCount; i++) {
                childInputs.add(childInputValues[i]);
                childTimings.add(childTimingValues[i]);
            }
        }
