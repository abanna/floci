...
class RdsControlPlaneTest {
    ...
    static void setup() {
        ...
        subnetIds = defaultSubnetIds(Region.US_EAST_1);
    }
    ...
    void sdkScopesTheSameDbProxyNameBySignedRegion() {
        ...
        try (RdsClient east = rdsClient(Region.US_EAST_1);
            ...
            var eastProxy = createIamProxy(
                    east, regionalProxyName, defaultSubnetIds(Region.US_EAST_1));
            var westProxy = createIamProxy(
                    west, regionalProxyName, defaultSubnetIds(Region.US_WEST_2));
    ...
    private static CreateDbProxyResponse createIamProxy(
            RdsClient client, String name, List<String> regionalSubnetIds) {
        return client.createDBProxy(b -> b
                .dbProxyName(name)
                .engineFamily("POSTGRESQL")
                .roleArn("arn:aws:iam::000000000000:role/rds-proxy-regional")
                .vpcSubnetIds(regionalSubnetIds)
                .defaultAuthScheme("IAM_AUTH"));
    }

    private static List<String> defaultSubnetIds(Region region) {
        List<Subnet> subnets;
        try (Ec2Client ec2 = ec2Client(region)) {
            DescribeSubnetsResponse response = ec2.describeSubnets();
            subnets = response.subnets().stream()
                    .filter(subnet -> Boolean.TRUE.equals(subnet.defaultForAz()))
                    .sorted((left, right) -> left.subnetId().compareTo(right.subnetId()))
                    .limit(2)
                    .toList();
        }
        assertThat(subnets)
                .as("default subnets in %s", region.id())
                .hasSize(2);
        assertThat(subnets)
                .extracting(Subnet::availabilityZone)
